...
static bool handle_socket_read(struct rtmp_stream *stream, uint64_t last_send_time)
{
	char discard[16384];
...
static bool handle_socket_eof(struct rtmp_stream *stream, uint64_t last_send_time)
{
	if (last_send_time) {
		uint32_t diff = (os_gettime_ns() / 1000000) - last_send_time;
...
static inline void socket_thread_macos_internal(struct rtmp_stream *stream)
{
	...
	if (setsockopt(stream->rtmp.m_sb.sb_socket, IPPROTO_TCP, TCP_NOTSENT_LOWAT, &notsent_lowat,
	...
	for (;;) {
		...
		for (int i = 0; i < nev; i++) {
			...
			if (ev->filter == EVFILT_READ) {
				if (ev->flags & EV_EOF) {
					if (!handle_socket_eof(stream, last_send_time)) {
						close_kqueue(stream, kq);
						return;
					}
				} else {
					if (!handle_socket_read(stream, last_send_time)) {
						close_kqueue(stream, kq);
						return;
					}
				}
